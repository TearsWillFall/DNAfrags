...
plot_fragments=function(bin_path="tools/samtools/samtools",file="",verbose=FALSE){
  ...
  if(verbose){
    print(paste(paste0("./",bin_path),"view",file," | awk '{sub(\"^-\", \"\", $9); print $9}'|sort | uniq -c >",paste0(sample_name,"_fragment_length.txt")))
  }
  system(paste(paste0("./",bin_path),"view",file," | awk '{sub(\"^-\", \"\", $9); print $9}'|sort | uniq -c >",paste0(sample_name,"_fragment_length.txt")))
}
