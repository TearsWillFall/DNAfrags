...
get_fragment_length_bed=function(bin_path="tools/samtools/samtools",bam="",bed="",max_frag_length=1000,mapq=10,threads=1,output_dir="",verbose=FALSE){
  ...
  data=data.frame(chr=ref_data[,1],r_start=(ref_data[,2]+1),r_end=(ref_data[,3]+1)) %>% dplyr::mutate(f_start=(ifelse((r_start-max_frag_length)<1),1,r_start-max_frag_length),f_end=(r_end+max_frag_length),r_id=ref_data[,4]) %>% dplyr::filter(!grepl("_",chr))
  print(data)
