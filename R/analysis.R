...
trim_fragments=function(bin_path="~/tools/bamUtil/bam",quality=33,first_base="",last_base="",file_R1="",file_R2="",output_dir=""){
  ...
  if(!dir.exists(output_dir)){
    ...
    mapply(x=c(file_R1,file_R2),y=c(1,2),verbose=verbose,bin_path=bin_path,quality=quality,first_base=first_base,last_base=last_base,FUN=fun)
  ...
  if(verbose){
    print(paste(bin_path,paste0("-Q",quality),"-f",first_base,"-l",last_base,"-z -i",file_R1,"-o",paste0(ULPwgs::get_sample_name(file_R1),".fastq.gz")))
  }
  system(paste(bin_path,paste0("-Q",quality),"-f",first_base,"-l",last_base,"-z -i",file_R1,"-o",paste0(ULPwgs::get_sample_name(file_R1),".fastq.gz")))
  }
}
